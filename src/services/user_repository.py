...
class UserRepository:
    ...
    def login(self, user_id_model: model.UserID, password_model: model.Password) -> bool:
        ...
        Args:
            user_id_model (model.UserID): The ID of the user attempting to log in.
            password (model.Password): The password provided by the user attempting to log in.
        ...
        with sqlmodel.Session(self.__engine) as session:
            account: model.Account | None = session.get(model.Account, user_id_model.user_id)
        hashed_password = model.HashedPassword(hashed_password=account.hashed_password)
        if (account is not None) and (hashed_password.verify(password_model)):
            return True
        return False
